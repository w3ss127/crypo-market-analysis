...
class CompanyIntelligenceProvider:
    ...
    async def get_intelligence(
        ...
        try:
            ...
            company_data, error_message = await self._get_company_data_with_extenal_api_call(ticker, analysis_type, additional_params)
            ...
            bt.logging.error(
                f"Error getting intelligence for {ticker} / {analysis_type}: {e}"
            )

            return IntelligenceResponse(
                success=False, data={"company": {"ticker": ticker}}, errorMessage=str(e)
            )
    ...
    async def _get_company_data(
                ...
                async with session.get(url, params=params, headers=headers) as response:
                    if response.status == 200:
                        ...
                        if not isinstance(data, dict):
                            ...
                            }, "Invalid downstream response format"

                        if (
                            "error" in data
                            or ("status" not in data or data["status"] != "ok")
                            or not "result" in data
                        ):
                            bt.logging.warning(f"API error for {ticker}: {data}")
                            return None, (
                                data["message"]
                                if "message" in data
                                else "Unknown error from downstream API"
                            )

                        result: dict = data["result"]
                        if not isinstance(result, dict):
                            bt.logging.warning(
                                f"Invalid result format for {ticker}: {result}"
                            )
                        ...
                        if attempt == max_retries - 1:
                            return (
                                None,
                                f"Downstream API responded with HTTP {response.status} for {ticker}",
                            )
        ...
        return (
            None,
            f"Max retries exceeded while fetching data from downstream API for {ticker}",
        )
